...
import ru.vladik.dnevnikru.databinding.DateBinding
import ru.vladik.dnevnikru.ext.getDp
import ru.vladik.dnevnikru.util.DateHelper

/**

Адаптер для отображения дней в виде списка. Расширяет класс [RecyclerDifferViewListAdapter] и работает с элементами типа [DateHelper.Day].
...
class DayPickerAdapter(data: MutableList<DateHelper.Day>?) : RecyclerDifferViewListAdapter<DayPickerAdapter.ViewHolder, DateHelper.Day>(data) {
    ...
    * @return ViewHolder для элемента списка.
    ...
    Вызывается, когда адаптер присоединяется к [RecyclerView].
    Устанавливает отступ между элементами списка и уведомляет адаптер о необходимости обновления списка.
    * @param recyclerView RecyclerView, к которому присоединен адаптер.
    ...
    Вызывается при замене списка элементов новым списком.
    Сбрасывает выбранный элемент списка.
    *@param data новый список элементов.
    ...
    Вызывается при привязке ViewHolder к элементу списка.
    Задает параметры отображения элемента списка.
